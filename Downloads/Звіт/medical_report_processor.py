...
class MedicalReportProcessor:
    ...
    def load_full_data(self):
        ...
        try:
            ...
            column_mapping = {
                'Унікальний код пацієнта': 'patient_id',
                'Unnamed: 3': 'doctor_name',
                'Unnamed: 4': 'doctor_position',
                'Приорітет': 'priority',
                'Unnamed: 6': 'amount'
            }
            
            # Знаходимо фактичні назви стовпців
            actual_mapping = {}
            for key, value in column_mapping.items():
                for col in df.columns:
                    if key.lower() in str(col).lower() or col == key:
    ...
    def load_report_data(self):
        ...
        try:
            ...
            column_mapping = {
                'ID пацієнта': 'patient_id',
                'До сплати, грн': 'amount'
            }
    ...
    def get_unique_doctors(self, df_full):
        ...
        doctors = [doc for doc in doctors if str(doc).strip() != '']
        
        print(f"Знайдено унікальних лікарів: {len(doctors)}")
        for i, doctor in enumerate(doctors[:5]):  # Показуємо перших 5
            print(f"  {i+1}. {doctor}")
    ...
    def process_doctor_data(self, df_full, df_report, doctor_name):
        ...
        result = {
            ...
            'doctor_position': doctor_data_full['doctor_position'].iloc[0] if 'doctor_position' in doctor_data_full.columns else 'Невідомо',
    ...
    def create_excel_report(self, results):
        ...
        for result in results:
            summary_data.append({
                ...
                'Загальна сума до сплати (грн)': result['total_amount']
            })
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            for result in results:
                sheet_name = result['doctor_name'][:30]  # Обмежуємо довжину назви
